...
### DET-020 — First-person practice exclusion catalog `ru-mvp-5` (D-075)
...
- `D-075`: immutable `ru-mvp-5` plus `NEG-ADV-020` first-person practice exclusion (`DET-020`).
...
## A.8 `ru-mvp-5` addition

Version `ru-mvp-5` copies every v4 rule and adds exactly the row below. Historical v1–v4 rows/checksums remain unchanged. Bare `\bпод ключ\b` is not added.

```text
| Rule ID | Priority | Target | Dimension | Weight | Exact pattern | Explanation code |
|---|---:|---|---|---:|---|---|
| `NEG-ADV-020` | 139 | `advertising` | `hard_exclusion` | 0 | `(?s)^(?!.*\b(?:нужен|ищу|требуется|посоветуйте|порекомендуйте)\b.{0,80}\b(?:сайт|лендинг|бот|интеграц|автоматизац|парсер|магазин)).*\b(?:занимаюсь|разрабатываю|пишу)\b.{0,80}\b(?:сайт(?:ов|а|ы)?|лендинг|бот|интеграц|автоматизац|парсер|магазин|нейросет)` | `advertising_first_person_practice` |
```

Frozen AT-DET-020 corpus (полный текст, без многоточия; fixtures MUST copy these strings, not sqlite):

NEG-R18-A (`biznes_spbchat1`, 240 code points):
```
Я Иван! Занимаюсь разработкой сайтов под ключ!

Не просто сайты, а при необходимости провожу маркетинговые исследования, с целью выявить ключевые факторы роста сайта в поисковых системах, а также быть лучше конкурентов!

• Одностраничные и
```

NEG-R18-B (`spbdvij`, 240 code points):
```
Всем привет! Меня зовут Сергей Горшенин.

Занимаюсь внедрением нейросетей в бизнес — на понятном языке, без модных слов и магии.

Если честно, нейросети — не панацея. Это инструмент. Но если разобраться, где именно у вас утекает время, он з
```
...
Сейчас работаю в команде разработчиков над развитием в2в площадки объектов недвижимости ITRIELT и реализую
```

POS-1: `Нужен сайт на Tilda, бюджет 150000`
POS-2: `Ищу разработчика telegram-бота`
POS-3: `Нужен сайт под ключ`

...
| SEC-019 | Proxy host, port, username, password и raw WinINET value MUST существовать только в памяти процесса; persistence, logs, metrics, exports и UI запрещены. Redactor MUST закрывать proxy fields и URI credentials. |
...
| AT-SEC-019 | Proxy URI с credentials проходит через settings, logger и error paths | Ни host/port/username/password, ни raw URI не появляются в persistence или внешнем выводе; route/type/reason code остаются доступны |
...
Требования SEC-001—SEC-019 обязательны для всех runtime-модулей. Любой новый adapter, export или log sink обязан использовать централизованные secret provider и redactor; собственное чтение secret files модулем запрещено.
...
- `SEC-OUTPUT`: AT-SEC-009, AT-SEC-013, AT-SEC-014, AT-SEC-016, AT-SEC-018, AT-SEC-019.

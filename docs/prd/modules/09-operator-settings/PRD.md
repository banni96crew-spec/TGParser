...
| SET-006 | Система MUST поддерживать группы `telegram`, `discovery`, `collector`, `rules`, `scoring`, `notifications`, `storage`, `observability` и `ui`. Группа `notifications` MUST включать `notifications.delivery_mode` ∈ {`shadow`, `live`} с default `shadow` (D-047). |
...
| AT-SET-006 | Settings snapshot содержит все девять утверждённых групп; `notifications.delivery_mode` default равен `shadow`. |
...
- `SettingChange`: change ID, key, old value, new value, reason, changed_at, `change_source` ∈ {`ui`, `startup_seed`, `migration`, `system`}.

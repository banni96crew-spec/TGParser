...
| OBS-019 | Система MUST публиковать novelty/suppress/exhaustion metrics aligned with SRC-037/038 / D-076: `discovery_novel_presented_total`, `discovery_registry_suppressed_total`, `discovery_dismissed_suppressed_total`, `discovery_cooldown_suppressed_total` (unique peers hidden by the D-076 rule), `discovery_pool_exhausted_total{reason}`, `discovery_novelty_ratio` (omitted or null-equivalent when no quality presented). Labels MUST NOT включать query text, source title, username, run ID или Telegram ID. |
...
| OBS-023 | Telegram connection health MUST expose only `route=direct|system_proxy`, optional proxy type and closed reason code. Proxy endpoint and credentials MUST NOT appear in logs, health, metrics or UI. |
...
| AT-OBS-023 | Success/failure/reconnect with credentialed proxy fixture | Health/logs contain route/type/reason only and no endpoint or credentials |
...
- `OBS-MVP-METRICS`: AT-OBS-013, AT-OBS-016, AT-OBS-017, AT-OBS-019, AT-OBS-021, AT-OBS-022, AT-OBS-023.

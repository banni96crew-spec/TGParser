"""Startup must activate ru-mvp-4 via the real runtime seed boundary."""
...
@pytest.mark.asyncio
async def test_startup_catalog_activates_ru_mvp_4_from_active_v1(db_env) -> None:
    """DB with only active ru-mvp-1 becomes exactly one active ru-mvp-4."""
    ...
    async with session_scope() as session:
        ...
        assert set(by_slug) == {"ru-mvp-1", "ru-mvp-2", "ru-mvp-3", "ru-mvp-4"}
        active = [v for v in versions if v.state == "active"]
        assert len(active) == 1
        assert active[0].slug == "ru-mvp-4"
        assert active[0].checksum == catalog_checksum(SEED_RULES_RU_MVP_4)
        ...
        assert v4_rules == len(SEED_RULES_RU_MVP_4)
    ...
    async with session_scope() as session:
        ...
        active = [v for v in versions if v.state == "active"]
        assert len(active) == 1 and active[0].slug == "ru-mvp-4"
        total_rules = int(
        ...
        expected_rules = (
            len(SEED_RULES) + len(SEED_RULES_RU_MVP_2)
            + len(SEED_RULES_RU_MVP_3) + len(SEED_RULES_RU_MVP_4)
        )
        assert total_rules == expected_rules

        # Discovery run pins active v4 after startup.
        ...
        assert started.run.rule_set_checksum == catalog_checksum(SEED_RULES_RU_MVP_4)
...
async def test_startup_catalog_checksum_mismatch_fails_loudly(db_env) -> None:
    ...
    async with session_scope() as session:
        with pytest.raises(RuntimeError, match=r"ruleset_checksum_mismatch:ru-mvp-4"):
            await seed_startup_catalog(session)

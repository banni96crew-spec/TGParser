...
def test_03_full_diversity_allocation_and_global_fill() -> None:
    ...
    assert sum(item.selected_lane == "DIRECTORY" for item in reserved) == 3
    assert sum(item.selected_lane == "EXPLORATION" for item in reserved) == 1
    assert sum(item.selection_phase == "global_fill" for item in selected) == 18
...
def test_05_overlap_ownership_prefers_linked_once() -> None:
...
def test_08_directory_has_reserved_representation_over_25_search_candidates() -> None:
...
def test_21_reserved_admission_does_not_override_final_buyer_order() -> None:
    ...
    assert by_id[2].selection_phase == "diversity_reserved"
...
def test_integrated_business_fixture_over_25_candidates() -> None:
    ...
    assert {101, 102, 103, 104, 105, 106}.issubset(ids)

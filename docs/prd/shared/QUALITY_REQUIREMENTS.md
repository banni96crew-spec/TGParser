...
| `NFR-QLT-008` | Live usefulness gate for `ActiveClientChat v1` (D-070) | On the unmodified immutable profile/rules contract, a live Zero-Stars run returns ≥1 quality public megagroup and the operator explicitly confirms three linked evidence messages as actionable development requests. Automated tests, fixture PASS, API success or agent self-assessment cannot satisfy this criterion. After at most five owner-authorized runs without that evidence the objective remains not achieved; thresholds MUST NOT be relaxed. |

...
async def ensure_seed_keyword_profile(
    session: AsyncSession,
) -> ProfileWithVersion:
    """Load the seed implementation lazily to keep dependency direction acyclic."""
    from telegram_lead_discovery.source_discovery.profile_seed_service import (
        ensure_seed_keyword_profile as _ensure_seed,
    )

    return await _ensure_seed(session)

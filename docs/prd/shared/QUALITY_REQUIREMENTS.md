...
| `NFR-QLT-006` | Remediation calibration / discovery quality gates (single numeric source of truth, D-067 / D-076) | Fixed corpus: hot precision ≥ `0.80`; hot+warm precision ≥ `0.70`; purchase-intent/`direct_order` recall ≥ `0.75`; permanent dismiss suppress recurrence = `0`; in-run canonical dedupe ≤ `1` presentation / canonical / run; `novelty_ratio` and Jaccard **only** over canonical peers with `truth_status=quality`; after a run that presented ≥1 quality, `novelty_ratio ≥ 0.80` when a quality replacement pool exists; if no quality presented, novelty is not computed and MUST NOT fail the 0.80 gate; live Jaccard over all presented peers is void; UI bind `127.0.0.1` only |
| `NFR-QLT-007` | Working-client-search DET quality (D-070 supersedes D-068 discovery gate) | Versioned labeled corpora with honest provenance: C01–C20 = operator run13 sanitized (owner labels; 3+/17− — not population alone); run14 regression = `operator_run_14_sanitized_excerpt` (not population); T1–T5 = DET-A golden (`det_a_golden`). Report C* / R14 / T* / combined separately; combined precision ≥ `0.80` and recall ≥ `0.80`. Discovery thresholds and run gate are owned only by SRC-024/046/047. |
| `NFR-QLT-008` | Live usefulness gate for `ActiveClientChat v1` (D-070 / D-073..D-076) | Immutable live contract is profile v8 + `ru-mvp-5` + D-073..D-076. Live step 1 (empty `seed_refs`): `global_message`/`groups` buyer queries MUST show `request_count>0`; this does not replace SEARCH quality. A live Zero-Stars keyword run MUST return ≥1 quality public megagroup whose provenance SEARCH includes `global_message` in `discovery_channels`, and the operator MUST explicitly confirm three linked evidence permalinks as actionable development requests. Quality only from `operator_seed` satisfies SRC-055/AT-SRC-055, **not** this criterion. Automated tests, fixture PASS, API success or agent self-assessment cannot satisfy this criterion. D-070 numeric thresholds MUST NOT be relaxed. After at most five owner-authorized **empty-seed** runs without SEARCH quality the objective remains not achieved. |

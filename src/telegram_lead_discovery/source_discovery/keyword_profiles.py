...
from telegram_lead_discovery.source_discovery.keyword_profile_seed import (
    ...
    MAX_QUERY_LEN,
    MAX_REPLACEMENT_DIRECTORY_QUERIES,
    MIN_DIRECTORY_QUERIES,
    MIN_POST_QUERIES,
    MIN_QUERY_LEN,
    MIN_REPLACEMENT_DIRECTORY_QUERIES,
...
def _validate_query(normalized: str) -> None:
    ...
    if length < MIN_QUERY_LEN or length > MAX_QUERY_LEN:
        raise ProfileValidationError(f"query_length_out_of_range:{length}")
...
def normalize_profile_queries(
    *,
    ...
    replacements = _normalize_query_list(
        replacement_directory_queries,
        min_count=MIN_REPLACEMENT_DIRECTORY_QUERIES,
        max_count=MAX_REPLACEMENT_DIRECTORY_QUERIES,
        field="replacement_directory_queries",
    )
    all_queries = (*posts, *directories, *replacements)
    if len(set(all_queries)) != len(all_queries):
        raise ProfileValidationError("duplicate_query_across_lists")
    ...
    return NormalizedProfileQueries(
        ...
        replacement_directory_queries=replacements,
...
def build_seed_normalized_profile() -> NormalizedProfileQueries:
    return normalize_profile_queries(
        ...
        replacement_directory_queries=SEED_DIRECTORY_REPLACEMENT_QUERIES,
...
from telegram_lead_discovery.source_discovery.keyword_profile_selection import (  # noqa: E402
